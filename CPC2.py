...
def __main():
    ...
    Common_group.add_option("-i", dest="fasta", help="input sequence in fasta format [Required]", metavar="FILE",
                            type="string", default=None)
    Common_group.add_option("-o", dest="outfile", help="output file [Default: cpc2output.txt]", metavar="FILE",
                            type="string", default="cpc2output.txt")
    Common_group.add_option("-r", dest="reverse", help="also check the reverse strand [Default: FALSE]",
                            action="store_true")
    ...
    if options.ORF:
        ...
        output_orf = 0
    if calculate_potential(options.fasta, strand, output_orf, options.outfile, options.libsvm_bin):
        return 1
...
def calculate_potential(fasta, strand, output_orf, outfile, libsvm_bin):
    '''
    ...
    if output_orf == 1:
        my_header = ["#ID", "transcript_length", "peptide_length", "Fickett_score", "pI", "ORF_integrity", "ORF_Start",
                     "coding_probability", "label"]
    else:
        my_header = ["#ID", "transcript_length", "peptide_length", "Fickett_score", "pI", "ORF_integrity",
                     "coding_probability", "label"]
    ...
    for seq in seqio.fasta_read(fasta):
        ...
        ftmp_feat.write(
            "\t".join(map(str, [seqid, len(seqRNA), pep_len, fickett_score, isoelectric_point, orf_fullness])) + "\n")
        ftmp_svm.write("".join(map(str, ["999", " 1:", pep_len, " 2:", fickett_score, " 3:", isoelectric_point, " 4:",
                                         orf_fullness])) + "\n")
    ftmp_result.close()
    ftmp_feat.close()
    ftmp_svm.close()
    ...
    # data_dir = pkg_resources.resource_dir(__name__,) + "/../data/"
    cpc2_model, cpc2_range = seqio.get_data()
    app_svm_scale = libsvm_bin + "/svm-scale"
    app_svm_predict = libsvm_bin + "/svm-predict"

    cmd = app_svm_scale + ' -r ' + cpc2_range + ' ' + outfile + '.tmp.1 > ' + outfile + '.tmp.2 &&'
    cmd = cmd + app_svm_predict + ' -b 1 -q ' + outfile + '.tmp.2 ' + cpc2_model + ' ' + outfile + '.tmp.out'
    ...
    exitstatus = subprocess.check_call(cmd, shell=True)
